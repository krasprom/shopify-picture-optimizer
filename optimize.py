...
def compose_square(img, mask, padding):
    """Кадрирует объект по его границам и центрирует в квадрате (натуральный масштаб).
    ...
    Маска используется ТОЛЬКО для определения границ объекта (bbox). Пиксели
    исходника не заменяются — фон внутри bbox остаётся оригинальным, чтобы
    неточная сегментация не отрезала часть самого товара.
    ...
    Возврат: PIL.Image (RGB), квадрат, объект по центру, белые поля-рамка.
    """
    rgb = img.convert("RGB")
    ...
    if bbox is None:
        ...
        # Обрезаем исходник по границам объекта; фон не трогаем.
